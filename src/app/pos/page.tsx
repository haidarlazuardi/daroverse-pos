...
function QueueCard({ order: o, onServed }: { order: any; onServed: () => void }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-xl mb-2">
      {/* Swipe bg */}
      <div className="absolute inset-0 flex items-center justify-end pr-5 rounded-xl" style={{ background: 'var(--brand)' }}>
      ...
      <div className="relative bg-white border rounded-xl"
        style={{
          ...
          transition: leaving ? 'transform 0.3s ease-in' : tx === 0 ? 'transform 0.15s ease-out' : undefined,
        }}
        onTouchStart={e => { startX.current = e.touches[0].clientX; }}
        onTouchMove={e => { const dx = e.touches[0].clientX - startX.current; if (dx > 0) setTx(Math.min(dx, 120)); }}
        ...
        {/* Expanded items */}
        {expanded && (
          <div className="border-t px-3 pt-2 pb-3 space-y-1" style={{ borderColor: 'var(--border)' }}>
            {(o.items || []).map((item: any, i: number) => (
              <div key={i} className="flex justify-between text-sm">
                <span className="text-gray-700"><span className="font-bold">{item.quantity}×</span> {item.product?.name}</span>
                <span className="text-gray-400 text-xs">{formatCurrency(item.price * item.quantity)}</span>
              </div>
            ))}
            <div className="flex justify-between pt-1.5 border-t text-xs font-semibold" style={{ borderColor: 'var(--border)' }}>
              <span className="text-gray-400">Total</span>
              <span style={{ color: 'var(--brand)' }}>{formatCurrency(o.total)}</span>
            </div>
            <p className="text-center text-xs text-gray-300 pt-1">geser kanan untuk selesaikan →</p>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="pos-grid bg-gray-50">
      ...
      <div className="flex flex-col h-screen overflow-hidden">
        <header className="bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-2 flex-shrink-0">
          ...
          {/* Staff Hub shortcut — mobile only, menggantikan sidebar */}
          <button onClick={() => router.push('/staff-hub')}
            className="lg:hidden flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-bold border"
          ...
          <button onClick={() => setShowCart(true)} className="hidden relative p-2.5 bg-green-600 text-white rounded-xl">
        ...
        <div className="flex-1 overflow-y-auto p-4" style={{ paddingBottom: cart.itemCount() > 0 ? '80px' : '16px' }}>
          {/* Floating cart bar — mobile only */}
          {cart.itemCount() > 0 && (
            <button
              onClick={() => setShowCart(true)}
              className="lg:hidden fixed bottom-0 left-0 right-0 z-40 flex items-center justify-between px-5 py-4 font-bold text-white"
              style={{ background: 'var(--brand)', boxShadow: '0 -4px 20px rgba(0,0,0,0.15)' }}>
              ...
              <span className="font-black text-lg">{formatCurrency(cart.total())}</span>
            </button>
          )}
          {loading ? <Loader /> : (
      ...
      <div className={clsx('bg-white border-l border-gray-200 flex flex-col h-screen', 'max-lg:fixed max-lg:inset-0 max-lg:z-50 max-lg:transition-transform max-lg:duration-300', showCart ? 'max-lg:translate-x-0' : 'max-lg:translate-x-full')}>
        <div className="px-3 py-3 border-b border-gray-200 flex items-center gap-2 flex-shrink-0">
          {step === 'cart' ? (<>
            {/* Tab toggle */}
            <div className="flex flex-1 bg-gray-100 rounded-xl p-1 gap-1">
              ...
              <button onClick={() => setRightTab('queue')}
                className={clsx('flex-1 py-1.5 text-sm font-semibold rounded-lg transition-all flex items-center justify-center gap-1', rightTab === 'queue' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500')}>
                Queue
                {queueCount > 0 && (
                  <span className="min-w-[18px] h-[18px] rounded-full text-xs font-black text-white flex items-center justify-center px-1 bg-red-500">{queueCount}</span>
                )}
              </button>
            </div>
            <button onClick={() => setShowCart(false)} className="lg:hidden p-1.5 hover:bg-gray-100 rounded-lg flex-shrink-0">
