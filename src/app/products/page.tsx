  ...
  function printMenuResep() {
    ...
    const pages = Object.entries(grouped).map(([cat,items]) => {
      const cards = items.map(p => {
        ...
        return `<div class="card"><div class="ctop"><div><div class="rname">${p.name}</div></div><div style="text-align:right"><div class="rprice">Rp ${p.price.toLocaleString('id-ID')}</div>${margin!==null?`<div class="mbadge">Margin ${margin}%</div>`:''}</div></div><div class="body"><div class="ci"><div class="lbl">Bahan</div>${ingr}</div><div class="cs"><div class="lbl">Cara Pembuatan</div>${steps}</div></div></div>`;
      }).join('');
      return `<div class="page"><div class="hdr"><div style="display:flex;align-items:center"><div class="logo">S</div><div class="title">Menu &amp; Resep</div></div><div style="font-size:7pt;color:#888">${now}</div></div><div class="cat">${cat} <span style="font-size:7.5pt;opacity:0.7;font-weight:400">${items.length} menu</span></div>${cards}<div class="ftr"><span>SOEKA HOUSE — Recipe Book</span><span>${now}</span></div></div>`;
    }).join('');
