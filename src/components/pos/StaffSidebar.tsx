...
function TakeForm({ingredients,busy,setBusy,onDone,stockAt}:any) {
  ...
  return <FW><Picker items={raw} value={id} onChange={setId} placeholder="Pilih bahan"/>{ing&&<p className="text-xs text-gray-400">Stok {from}: {stockAt(id,from)} {ing.unit}</p>}<div><p className="text-xs font-bold text-gray-500 mb-1">Dari</p><LP value={from} set={setFrom} locs={['GUDANG','BAR','KITCHEN']}/></div><div><p className="text-xs font-bold text-gray-500 mb-1">Ke</p><LP value={to} set={setTo} locs={['GUDANG','BAR','KITCHEN']}/></div><input type="number" value={qty} onChange={e=>setQty(e.target.value)} className="input text-sm" placeholder={`Jumlah (${ing?.unit||'unit'})`}/><button onClick={submit} disabled={busy||!id||!qty||from===to} className="btn btn-primary btn-sm w-full">{busy?'Proses...':'Transfer'}</button></FW>;
}
...
function WasteForm({ingredients,busy,setBusy,onDone,stockAt}:any) {
  ...
  return <FW><Picker items={ingredients} value={id} onChange={setId} placeholder="Pilih bahan"/>{ing&&<p className="text-xs text-gray-400">Stok {loc}: {stockAt(id,loc)} {ing.unit}</p>}<LP value={loc} set={setLoc} locs={['BAR','KITCHEN','GUDANG']}/><input type="number" value={qty} onChange={e=>setQty(e.target.value)} className="input text-sm" placeholder={`Jumlah dibuang`}/><input value={reason} onChange={e=>setReason(e.target.value)} className="input text-sm" placeholder="Alasan (opsional)"/><button onClick={submit} disabled={busy||!id||!qty} className="btn btn-sm w-full text-white" style={{ background:'#dc2626' }}>{busy?'Proses...':'Catat Waste'}</button></FW>;
}
...
function Opname({canApply,busy,setBusy,onDone}:any) {
  ...
  return <div className="p-3 space-y-2">{entries.map((e,i)=>(<div key={i} className="rounded-lg border border-gray-100 p-2.5 space-y-1.5"><select value={e.ingredientId} onChange={ev=>upd(i,'ingredientId',ev.target.value)} className="input w-full text-sm"><option value="">Pilih bahan</option>{ings.map((ing:any)=><option key={ing.id} value={ing.id}>{ing.name}</option>)}</select><div className="flex gap-1">{['GUDANG','BAR','KITCHEN'].map(l=><button key={l} onClick={()=>upd(i,'location',l)} className="flex-1 py-1 rounded text-xs font-bold border" style={{ borderColor:e.location===l?'var(--brand)':'#e5e7eb',color:e.location===l?'var(--brand)':'#9ca3af' }}>{l}</button>)}</div><input type="number" value={e.actualQty} onChange={ev=>upd(i,'actualQty',ev.target.value)} className="input text-sm" placeholder="Qty aktual"/></div>))}<button onClick={()=>setEntries(p=>[...p,{ingredientId:'',location:'BAR',actualQty:''}])} className="btn btn-secondary btn-sm w-full text-xs">+ Tambah</button><button onClick={submit} disabled={busy} className="btn btn-primary btn-sm w-full">{busy?'...':canApply?'Terapkan':'Simpan'}</button></div>;
}
function MenuView() {
  ...
  return <div className="p-3"><input value={search} onChange={e=>setSearch(e.target.value)} className="input text-sm mb-2" placeholder="Cari menu..."/><div className="space-y-1.5">{filtered.map(p=>(<div key={p.id} className="flex items-center justify-between rounded-lg p-2.5 border border-gray-100 bg-gray-50"><p className="font-semibold text-xs text-gray-900">{p.name}</p><p className="font-bold text-xs" style={{ color:'var(--brand)' }}>{formatCurrency(p.price)}</p></div>))}</div></div>;
}
